...
function sendBrevoSMS(to, content) {
  ...
  const payload = JSON.stringify({
    sender:    'MWH-Service', // max 11 chars, alphanumeric
...
async function sendConfirmationSMS(booking) {
  const msg = `Motowarehouse Service: Your ${SERVICE_LABELS[booking.serviceType]} is confirmed for ${booking.date} at ${booking.time}. Ref: ${booking.ref}. 40 Athinon Str, Strovolos. Tel: 22328788`;
  await sendBrevoSMS(booking.phone, msg);
}

async function sendCancellationSMS(booking) {
  const msg = `Motowarehouse Service: Your booking ${booking.ref} on ${booking.date} at ${booking.time} could not be confirmed. Please call 22328788 to reschedule.`;
  await sendBrevoSMS(booking.phone, msg);
}

async function sendReminderSMS(booking) {
  const msg = `Motowarehouse Service: Reminder – your ${SERVICE_LABELS[booking.serviceType]} is in ~2 hours at ${booking.time}. 40 Athinon Str, Strovolos. Tel: 22328788`;
  await sendBrevoSMS(booking.phone, msg);
}

async function sendRescheduleSMS(booking) {
  const msg = `Motowarehouse Service: Your booking ${booking.ref} has been rescheduled to ${booking.date} at ${booking.time}. Tel: 22328788`;
  await sendBrevoSMS(booking.phone, msg);
}
